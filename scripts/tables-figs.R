...
preds_18sv9 <- paste(stbl_dir, '18sv9-ss/loo-preds.rds', sep = '') |>
  read_rds() |>
  mutate(cruise = obs.id,
         cruise.ym = ym(cruise),
         year = year(cruise.ym),
  ...
  geom_path(aes(y = obs.ss), 
            linewidth = 0.3) + 
  geom_path(aes(y = pred.ss), 
            color = 'blue',
            linetype = 'dashed') +
  geom_ribbon(aes(ymin = pred.ss.qlo, ymax = pred.ss.qhi),
              fill = 'blue',
              alpha = 0.2) +
  facet_grid(species~.) +
  labs(y = NULL, x = NULL, title = '18SV9') +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_line(color = 'black', linewidth = 0.1))

preds_18sv4 <- paste(stbl_dir, '18sv4-ss/loo-preds.rds', sep = '') |>
  read_rds() |>
  mutate(cruise = obs.id,
         cruise.ym = ym(cruise),
         year = year(cruise.ym),
  ...
  geom_ribbon(aes(ymin = pred.ss.qlo, ymax = pred.ss.qhi),
              ...
              alpha = 0.2) +
  facet_grid(species~.) +
  labs(y = NULL, x = NULL, title = '18SV4') +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_line(color = 'black', linewidth = 0.1))
  
preds_16s <- paste(stbl_dir, '16s-ss/loo-preds.rds', sep = '') |>
  read_rds() |>
  mutate(cruise = obs.id,
         cruise.ym = ym(cruise),
         year = year(cruise.ym),
  ...
  geom_ribbon(aes(ymin = pred.ss.qlo, ymax = pred.ss.qhi),
              ...
              alpha = 0.2) +
  facet_grid(species~.) +
  labs(y = 'sightings per 1000km', x = NULL, title = '16S') +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_line(color = 'black', linewidth = 0.1))


preds_inner <- preds_16s + preds_18sv4 + preds_18sv9
ggsave(preds_inner, filename = paste(out_dir, 'preds-inner.png', sep = ''),
       width = 8, height = 4, units = 'in', dpi = 400)


preds_16s_outer <- read_rds(paste(val_dir, '16s-ss/loo-preds-outer.rds', sep = '')) |>
  inner_join(read_rds(paste(val_dir, '16s-ss/best-settings.rds', sep = ''))) |>
  mutate(cruise = outer.id,
         cruise.ym = ym(cruise),
         year = year(cruise.ym),
         species = factor(species, 
                          levels = c('bm', 'bp', 'mn'),
...
preds_18sv4_outer <- read_rds(paste(val_dir, '18sv4-ss/loo-preds-outer.rds', sep = '')) |>
  inner_join(read_rds(paste(val_dir, '18sv4-ss/best-settings.rds', sep = ''))) |>
  mutate(cruise = outer.id,
         cruise.ym = ym(cruise),
         year = year(cruise.ym),
         species = factor(species, 
  ...
  geom_path(aes(y = pred.ss), 
            ...
            linetype = 'dashed') +
  facet_grid(species~.) +
  labs(y = 'sightings per 1000km', x = NULL, title = '18sv4') +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_line(color = 'black', linewidth = 0.1))

preds_18sv9_outer <- read_rds(paste(val_dir, '18sv9-ss/loo-preds-outer.rds', sep = '')) |>
  inner_join(read_rds(paste(val_dir, '18sv9-ss/best-settings.rds', sep = ''))) |>
  mutate(cruise = outer.id,
         cruise.ym = ym(cruise),
         year = year(cruise.ym),
         species = factor(species, 
  ...
  geom_path(aes(y = pred.ss), 
            ...
            linetype = 'dashed') +
  facet_grid(species~.) +
  labs(y = 'sightings per 1000km', x = NULL, title = '18sv9') +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_line(color = 'black', linewidth = 0.1))

preds_outer <- preds_16s_outer + preds_18sv4_outer + preds_18sv9_outer
ggsave(preds_outer, filename = paste(out_dir, 'preds-outer.png', sep = ''),
       width = 8, height = 4, units = 'in', dpi = 400)
