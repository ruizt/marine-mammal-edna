...
stbl_dir <- 'rslt/stability-selection/18sv9/'
out_dir <- 'rslt/nested-validation/18sv9/'
dir_create(out_dir)
paste(out_dir, 'inner-spls-fits/', sep = '_') |> dir_create()

## DATA INPUTS -----------------------------------------------------------------

# read in and combine 18sv9 data and density estimates
...
partitions_nested <- paste(data_dir, 
                           '_combined-partitions/nested-partitions-18sv9-dens.rds', 
                           sep = '') |> 
  read_rds()
