...
stbl_dir <- 'rslt/stability-selection/18sv9-ss/'
...
fit_df <- fitted_models |>
  ...
  mutate(ss.fit = exp(lr.fit + seasonal.mean),
         lr.resid = lr.obs - lr.fit,
         ss.resid = ss.obs - ss.fit) |>
  select(species, cruise, ncomp, starts_with('lr'), starts_with('ss'))
...
model_metrics <- fit_df |>
  ...
  unnest(n.asv) |>
  select(species, n.asv, ends_with('.lr'), ends_with('.ss'))
...
save(list = c('whales',
              ...
              'fit_df',
              'model_metrics'),
     file = paste(out_dir, 'fitted-models-18sv9.RData', sep = ''))
