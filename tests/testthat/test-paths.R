...
test_that("core processed data files exist", {
  ...
  expect_true(file.exists("data/ncog16s.RData"))
  expect_true(file.exists("data/density-estimates.RData"))
})

test_that("line transect data files exist", {
  skip_if_no_data("data/sample_table.Rds")
  expect_true(file.exists("data/sample_table.Rds"))
  expect_true(file.exists("data/region_table.Rds"))
  expect_true(file.exists("data/obs_table.Rds"))
})

test_that("always-present repository files exist", {
  expect_true(file.exists("data/example-data-16s.RData"))
  expect_true(file.exists("data/gshhg.RData"))
})

test_that("partition directories exist (if downloaded)", {
  skip_if_no_data("data/_partitions")
  expect_true(dir.exists("data/_partitions"))
  expect_true(dir.exists("data/_combined-partitions"))
})
